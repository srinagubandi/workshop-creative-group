...
export function TextManager({ token }: { token: string }) {
  ...
  return <div className="space-y-5">
    ...
    <div className="flex flex-wrap gap-2">
      {ROUTES.map(([path, label]) => <button key={path} onClick={() => changeRoute(path)} className={`px-3 py-2 rounded-lg text-sm font-semibold ${routePath === path ? "bg-[#1261ae] text-white" : "bg-gray-800 text-gray-300 hover:text-white"}`}>{label}</button>)}
    </div>
