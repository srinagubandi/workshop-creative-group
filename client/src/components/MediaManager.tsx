...
export function MediaManager({ token }: { token: string }) {
  ...
  return (
    <section>
      ...
      {mediaQuery.isLoading ? <p className="py-12 text-center text-gray-500">Loading media library…</p> : Object.keys(groups).length === 0 ? <p className="py-12 text-center text-gray-500">No {status} media yet.</p> : Object.entries(groups).map(([group, items]) => <div key={group} className="mb-8"><h3 className="mb-3 text-sm font-semibold uppercase tracking-wider text-gray-400">{group.replace(/-/g, " ")}</h3><div className="grid gap-4 md:grid-cols-2">{items.map((asset, index) => <MediaCard key={asset.id} asset={asset} token={token} onUpdate={(data) => update.mutate({ token, id: asset.id, ...data })} onPlacement={(data) => { const existing = asset.placements[0]; existing ? updatePlacement.mutate({ token, id: existing.id, ...data }) : addPlacement.mutate({ token, mediaId: asset.id, pageKey: data.pageKey || "gallery", category: data.category || undefined, client: data.client || undefined, project: data.project || undefined, sortOrder: 0 }); }} onPublish={() => publish.mutate({ token, id: asset.id })} onUnpublish={() => unpublish.mutate({ token, id: asset.id })} onArchive={() => archive.mutate({ token, id: asset.id })} onRestore={() => restore.mutate({ token, id: asset.id })} onReplace={event => handleFile(event, asset.id)} onMoveUp={() => move(asset, -1, items)} onMoveDown={() => move(asset, 1, items)} disableUp={index === 0} disableDown={index === items.length - 1} />)}</div></div>)}
    </section>
  );
}

function MediaCard({ asset, onUpdate, onPlacement, onPublish, onUnpublish, onArchive, onRestore, onReplace, onMoveUp, onMoveDown, disableUp, disableDown }: { asset: MediaAsset; token: string; onUpdate: (data: { title?: string; caption?: string; altText?: string }) => void; onPlacement: (data: { pageKey?: string; category?: string; client?: string; project?: string }) => void; onPublish: () => void; onUnpublish: () => void; onArchive: () => void; onRestore: () => void; onReplace: (event: ChangeEvent<HTMLInputElement>) => void; onMoveUp: () => void; onMoveDown: () => void; disableUp: boolean; disableDown: boolean }) {
