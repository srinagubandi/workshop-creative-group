...
export function MediaManager({ token }: { token: string }) {
  ...
  const refresh = () => { mediaQuery.refetch(); allMediaQuery.refetch(); utils.media.gallery.invalidate(); };
  const update = trpc.admin.updateMedia.useMutation({ onSuccess: () => { setNotice("Details saved successfully."); refresh(); }, onError: e => setError(e.message) });
  const addPlacement = trpc.admin.addMediaPlacement.useMutation({ onSuccess: () => { setNotice("Placement saved. This asset is now assigned and ready to publish."); refresh(); }, onError: e => setError(e.message) });
  const updatePlacement = trpc.admin.updateMediaPlacement.useMutation({ onSuccess: () => { setNotice("Placement saved. This asset is now assigned and ready to publish."); refresh(); }, onError: e => setError(e.message) });
  const publish = trpc.admin.publishMedia.useMutation({ onSuccess: () => { setNotice("Asset published successfully."); refresh(); }, onError: e => setError(e.message) });
  ...
  async function uploadFile(file: File, targetId?: number | null, originalFile?: File | null) {
    ...
    try {
      ...
      else { setStatus("draft"); setSearch(payload.asset.originalFilename || ""); setNotice("Upload complete. The new asset is shown below as a draft. Save its placement first, then publish it."); refresh(); }
    } catch (err) {
  ...
  return (
    <section>
      <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-4 mb-6">
        ...
        <label className="inline-flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl text-sm font-semibold text-white cursor-pointer disabled:opacity-50" style={{ background: "#7ebf31" }}>
