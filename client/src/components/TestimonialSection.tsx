...
function TestimonialQuote({ quote, quoteId }: { quote: string; quoteId: string }) {
  const [expanded, setExpanded] = useState(false);
  const canCollapse = quote.length > 320;
  const preview = canCollapse && !expanded ? `${quote.slice(0, 320).trimEnd()}…` : quote;

  return <div>
    <blockquote id={quoteId} className="font-serif text-base md:text-lg leading-relaxed text-gray-800">“{preview}”</blockquote>
    {canCollapse && (
      <button
        type="button"
        className="mt-4 text-sm font-semibold underline underline-offset-4 focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 rounded-sm"
        style={{ color: "#1260ae" }}
  ...
  return <section className="section-py bg-white" aria-labelledby="testimonial-heading">
    <div className="container">
      ...
      <div className={layout === "single" ? "max-w-4xl mx-auto" : "flex gap-6 overflow-x-auto pb-4 snap-x snap-mandatory"} role={layout === "single" ? undefined : "list"} aria-label={layout === "single" ? undefined : "Client testimonials"}>
        {testimonials.map((item, index) => <figure key={item.id} role={layout === "single" ? undefined : "listitem"} className={`rounded-2xl border border-gray-100 bg-gray-50 p-7 md:p-9 shadow-sm flex flex-col ${layout === "single" ? "" : "min-w-[min(100%,22rem)] md:min-w-[24rem] snap-start"}`}>
          ...
          <TestimonialQuote quote={item.quote} quoteId={`testimonial-quote-${item.id ?? index}`} />
